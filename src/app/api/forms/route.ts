...
import { ensureDefaultProject } from '@/lib/database/projects'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, description, email_notifications, notification_email, project_id } = body
    ...
    // Ensure user has a project - if no project_id provided, use/create default project
    let finalProjectId = project_id
    if (!finalProjectId) {
      finalProjectId = await ensureDefaultProject(user.id)
    }
    ...
    const formData: FormInsert = {
      ...
      email_notifications: !!email_notifications, // Use exact boolean value
      notification_email: email_notifications 
        ? (notification_email?.trim() || user.email || null)
